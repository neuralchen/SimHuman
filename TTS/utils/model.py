...
import TTS.hifigan as hifigan
from TTS.model import FastSpeech2, ScheduledOptim
...
def get_vocoder(config, device):
    ...
    if name == "MelGAN":
        ...
        with open("./TTS/hifigan/config.json", "r") as f:
            config = json.load(f)
        config = hifigan.AttrDict(config)
        vocoder = hifigan.Generator(config)
        if speaker == "LJSpeech":
            ckpt = torch.load("./TTS/hifigan/generator_LJSpeech.pth.tar", map_location=device)
        elif speaker == "universal":
            ckpt = torch.load("./TTS/hifigan/generator_universal.pth.tar", map_location=device)
